...
Intuition:
identify minimum cost to transform source to target. the mapping is recorded at original and changed ,with cost[i] as weight! with those matrix ,we can build the edge / graph ,then we perform Dijkstra on the char that are different in source / target . More conveniently, we compute all pairs shortest path before resolving queries for source to target cost! 
...
Complexity:
Time: 
O(26 ^ 3) nested fro loop  is near constant 
we take O (m) time to initialize edges, where m is length of the original arr
O(n) for resolve queries where n is the length of the source! 

Space:
O(1) for the matrix

Tags: #floyd_warshall #shortest_path #dijkstra 
