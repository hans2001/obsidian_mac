...
## [How do browsers render a web page](https://frontendbabel.info/articles/webpage-rendering-101/#how-do-browsers-render-a-web-page)
...
### [Repaint](https://frontendbabel.info/articles/webpage-rendering-101/#repaint)
...
### [Reflow](https://frontendbabel.info/articles/webpage-rendering-101/#reflow)
...
## [How browsers optimize rendering](https://frontendbabel.info/articles/webpage-rendering-101/#how-browsers-optimize-rendering)
...
As a result, we get 2 reflows instead of one. Because of this, you should group reading element properties together to optimize performance ([see a more detailed example on JSBin](http://jsbin.com/duhah/2/edit)).

There are situations when you have to trigger a forced reflow. Example: we have to apply the same property ("margin-left" for example) to the same element twice. Initially, it should be set to `100px`without animation, and then it has to be animated with `transition` to a value of `50px`. You can study this example [on JSBin](http://jsbin.com/qutev/1/edit) right now, but I'll describe it here in more detail.
...
## [Practical advice on optimization](https://frontendbabel.info/articles/webpage-rendering-101/#practical-advice-on-optimization)
...
- Try to simplify and optimize CSS selectors (this optimization is almost universally ignored by developers who mostly use CSS preprocessors). Keep nesting levels at a minimum. This is how CSS selectors rank according to their performance (starting from the fastest ones):
    ...
    8. Pseudoclasses and pseudoelements: `a:hover` You should remember that browsers process selectors from right to left, that's why the rightmost selector should be the fastest one — either `#id` or `.class`:
...
5. It is a good idea to disable complicated `:hover` animations while scrolling (e.g. by adding an extra "no-hover" class to ).
